...
let rec testmr n ls =
    ...
    then
        ...
        (* if can do move, TEST FIRST IF MOVE IS MR OR ML AND IS ON THE END AND MOVING AWAY FROM AN EMPTY SPACE. IF SO, SAY FALSE *)
            ...
            if (n=len-1) && (nth ls (len-2) = O) && (nth ls (len-3) = R)
            then (false,-1)
            else (true,n)
    else (false,-1)

let testml xs =
    let rec testml_aux n ls =
        let len = (List.length ls) in
        if n < len
        then
            if (ml xs (len-(n+1)) [] = [])
            then testml_aux (n+1) ls
            else
                if (n=len-1) && (nth ls (len-2) = O) && (nth ls (len-3) = L)
                then (false,-1)
                else (true,(len-(n+1)))
        else (false,-1)
    in testml_aux 0 (List.rev xs)
;;
...
(*  
    ...
    if not (penultimate item = O && last item = R) then
    test mr_end on last element of list
    if not (first item = L && second item = O) then
